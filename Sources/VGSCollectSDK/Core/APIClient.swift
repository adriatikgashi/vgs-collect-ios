...
class APIClient {
    ...
    func sendRequestBC(path: String, method: HTTPMethod = .post, value: String, completion block: ((_ response: VGSResponse) -> Void)? ) {
        
        let sendRequestBlock: (URL) -> Void = {requestURL in
            let url = requestURL.appendingPathComponent(path)
            self.sendRequestBC(to: url, method: method, value: value, completion: block)
        }
    ...
    private  func sendRequestBC(to url: URL, method: HTTPMethod = .post, value: String, completion block: ((_ response: VGSResponse) -> Void)? ) {
        ...
        // Setup URLRequest
        var request = URLRequest(url: url)
        request.httpBody = value.data(using: .ascii)
        request.httpMethod = method.rawValue
        request.allHTTPHeaderFields = headers
        // Send data
        URLSession.shared.dataTask(with: request) { (data, response, error) in
